...
load("data/merged_data_baseline_hpfs.Rdata")
surv_dict <- surv_dict %>% filter(subtype %in% c("all"))

##-----------------------------------------
## Predictors
##-----------------------------------------

source("scripts/source_predictiors_list_replication.R")

##-----------------------------------------
## Final data
##-----------------------------------------

tmp <- merged_data_hpfs %>% 
  subset(select=c("id", "study_ID", surv_dict$time, surv_dict$status, 
                  lifestyle_short_list,
                  apoe4_2cat_list, 
                  prs_list))
model_data <- merge(tmp, rf_imputed_data_hpfs, by="study_ID")
...
load("results/pred_met_candidate_hpfs.Rdata")
...
source("scripts/source_predictors_final_replication.R")
...
for (subset in names(model_data_list)){
  ...
  for (j in 1:dim(surv_dict)[1]){
    ...
    tmp_covar_list[["pred_met_all"]] <- c(lifestyle_APOE4_PRS_list, met_pred)
...
save.image("data/prediction_data_hpfs.RData")
