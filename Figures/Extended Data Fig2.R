...
pc_corr_dat <- pc_corr_dat %>%
  mutate(sign_pc1 = case_when(corr_pc1 > 0 ~ "Positive",
                              corr_pc1 <= 0 ~ "Inverse"),
         fdr_pc1 = p.adjust(p_pc1, "BH"),
         sig_pc1 = case_when(fdr_pc1 < 0.05 ~ "sig",
                             fdr_pc1 >= 0.05 ~ "non_sig"),
         abs_pc1 = abs(corr_pc1),
         sign_pc2 = case_when(corr_pc2 > 0 ~ "Positive",
                              corr_pc2 <= 0 ~ "Inverse"),
         fdr_pc2 = p.adjust(p_pc2, "BH"),
         sig_pc2 = case_when(fdr_pc2 < 0.05 ~ "sig",
                             fdr_pc2 >= 0.05 ~ "non_sig"),
...
table(pc_corr_dat$fdr_pc1 <= 0.05)
table(pc_corr_dat$fdr_pc2 <= 0.05)

p_fdr_thresh_pc1 <- max(pc_corr_dat$p_pc1[pc_corr_dat$fdr_pc1 <= 0.05], na.rm = TRUE)
p_fdr_thresh_pc2 <- max(pc_corr_dat$p_pc2[pc_corr_dat$fdr_pc2 <= 0.05], na.rm = TRUE)
...
pdf("results/extended/extended_fig2a_legend1.pdf", width = 25, height = 3, onefile = F)
legend_exdfig2a1 <- cowplot::get_plot_component(class_met, 'guide-box', return_all = TRUE)
grid.newpage()
grid.draw(legend_exdfig2a1[[1]])
dev.off()
...
mhtplot_pc1 <- ggplot(pc_corr_dat, aes(x=metabolite_name, y=-log10(p_pc1), fill=sign_pc1)) +
  ...
  geom_hline(yintercept = -log10(p_fdr_thresh_pc1), linetype="dashed") +
...
pdf("results/extended/extended_fig2a_legend2.pdf", width = 25, height = 3, onefile = F)
...
mhtplot_pc2 <- ggplot(pc_corr_dat, aes(x=metabolite_name, y=-log10(p_pc2), fill=sign_pc2)) +
  ...
  geom_hline(yintercept = -log10(p_fdr_thresh_pc2), linetype="dashed") +
  scale_fill_manual(values=c("#E67E22","#34495E")) +
  scale_x_discrete(limits=rev) +
...
pdf("results/extended/extended_fig2a.pdf", width = 11, height = 8, onefile = F)
...
gene_met_dat <- fread("results/gene_met_nhs_2023.txt") %>%
...
pdf("results/extended/extended_fig2b_legend1.pdf", width = 25, height = 3, onefile = F)
...
pdf("results/extended/extended_fig2b_legend2.pdf", width = 25, height = 3, onefile = F)
...
pdf("results/extended/extended_fig2b.pdf", width = 11, height = 8, onefile = F)
