...
diet_res <- read_xlsx("results/diet_gene_met_main_subgroup.xlsx") %>%
  ...
  filter(diet=="AMED_avg",
         (subgroup=="all" & covar=="full") | 
           (subgroup %in% c("APOE4_noncarrier","APOE4_carrier","APOE4_ncopy_1") & covar=="full") | 
           (subgroup %in% c("APOE4_ncopy_2") & covar=="full")) %>%
...
diet_inter_res <- read_xlsx("results/diet_gene_met_inter.xlsx") %>% filter(diet=="AMED_avg")
diet_inter_ind_var_res <- read_xlsx("results/diet_gene_met_inter_ind_var.xlsx")
...
for (hmdb_id in c("HMDB0000168", "HMDB0011103", "HMDB0010368", "HMDB0003357")) {
  ...
  for (apoe4_group in unique(merged_data$APOE4)) {
    ...
    tmp_mod <- glm(as.formula(paste(hmdb_id, "~ agemo + blddate + bmi + fast1 + endpoint + caco + hiedu3 + 
                      ...
                      sysbpm + hchol + alcocon + daykcal")), data=tmp_data)
...
pdf("results/figures/fig3e_call_out.pdf", width = 6, height = 2.5, onefile = F)
