...
all_data <- merge_metab_data(cohorts = "hpfs",
...
control_data <- merge_metab_data(cohorts = "hpfs",
...
substudy_data <- merge_metab_data(cohorts = "hpfs",
...
save.image("data/metabolites/metabolomics_data_raw_hpfs.RData")
...
load("data/metabolites/metabolomics_data_raw_hpfs.RData")
...
write.csv(mdata_final, "data/metabolites/metabolomics_data_full_hpfs.csv", na="", row.names=F, quote=F)

fdata_all <- fdata_sub[fdata_sub$source_data=="all",]; save(fdata_all, file="data/metabolites/fdata_all_hpfs.RData")
fdata_control <- fdata_sub[fdata_sub$source_data=="control",]; save(fdata_control, file="data/metabolites/fdata_control_hpfs.RData")
fdata_substudy <- fdata_sub[fdata_sub$source_data=="substudy",]; save(fdata_substudy, file="data/metabolites/fdata_substudy_hpfs.RData")

save(fdata_sub, file="data/metabolites/fdata_full_hpfs.RData")
save(fdata_sub_agg, file="data/metabolites/fdata_full_agg_hpfs.RData")

tmp1 <- fdata_sub %>%
  mutate(hmdb_id = gsub("\\*", "", hmdb_id)) %>%
  group_by(hmdb_id) %>%
  dplyr::summarise(mean_icc_final = mean(mean_icc, na.rm=T),
...
save(fdata, file="data/metabolites/fdata_hpfs.RData")

save(prdata_all, file="data/metabolites/prdata_hpfs.RData")
save(prdata_control, file="data/metabolites/prdata_control_hpfs.RData")
save(prdata_substudy, file="data/metabolites/prdata_substudy_hpfs.RData")
