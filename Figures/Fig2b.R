...
pdf("results/figures/fig2_legend.pdf", width = 25, height = 3, onefile = F)
...
pdf("results/figures/fig2b_legend.pdf", width = 25, height = 3, onefile = F)
...
inter_ind_var_mhtplot <- ggplot(ind_var_inter_all, aes(x=metabolite_name, y=-log10(p_fdr_inter_met_var))) +
  ...
  annotation_custom(grobTree(textGrob("Interaction with other AD/ADRD variants", x=0.98,  y=0.92, hjust=1,
                                      gp=gpar(fontsize=20)))) +
...
pdf("results/figures/fig2b_overall_findings.pdf", width = 11, height = 8, onefile = F)
