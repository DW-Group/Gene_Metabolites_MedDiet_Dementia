...
##-----------------------------------------
## Exd Fig. 2: Significant interaction by superclass  
##-----------------------------------------
...
pdf("results/extended/extended_fig2a.pdf", width = 7, height = 5, onefile = F)
...
##----------------------------------
## 2b: Individual variant interactions
##----------------------------------
...
pdf("results/extended/extended_fig2b.pdf", width = 7, height = 5, onefile = F)
ggplot(count_by_super_class, aes(y = super_class_metabolon, x = n, fill = factor(fdr_sig))) +
  geom_bar(stat = "identity", position = "stack", width = 0.6) +
  scale_fill_manual(values = c("1" = "#DAA520", "0" = "#ad4a44"), 
                    labels = c("FDR < 0.05", "P < 0.05 & FDR > 0.05")) +
